...
export default async function EditDisciplinePage({ params }: EditDisciplinePageProps) {
  ...
  const discipline = await disciplineService.getDiscipline(params.id)
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <DisciplineForm
          ...
          onSubmit={async (data) => {
            'use server'
            const response = await disciplineService.updateDiscipline(params.id, {
              ...data,
              courseId: params.courseId,
              order: discipline.order
            })
            if (!response.success) {
              throw new Error(response.error?.message || 'Erro ao atualizar disciplina')
            }
