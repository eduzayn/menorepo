 ...
 */
const MODULE_CONFIGS: Record<AppModule, ModuleNavigation> = {
  dashboard: {
    name: 'Dashboard',
    baseRoute: '/dashboard',
    routes: [
...
export function useNavigation() {
   ...
   */
  const getCurrentModule = useCallback((): AppModule | null => {
    const currentPath = location.pathname;
    
    const currentModule = Object.entries(MODULE_CONFIGS).find(([_, config]) => 
      currentPath.startsWith(config.baseRoute)
    );
    
    return currentModule ? currentModule[0] as AppModule : null;
  }, [location.pathname]);
